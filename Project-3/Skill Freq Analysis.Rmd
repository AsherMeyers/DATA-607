---
title: "Data 607 Project 3: Data Analysis"
author: "Asher Meyers & Gurpreet Singh"
date: "March 25, 2016"
...
We'll bring back our Twitter data here, so that we can look at the results of the two datasets together.

We'll consider both sets of results in barplot form, and use the rainbow() function to distinguish one bar from another.


```{r}

barplot(twitSort$t_freq, main = "Twitter Mentions of Data Science Skills", xlab = "Skill", ylab = "# of mentions", col = rainbow(nrow(twitSort)))
...
barplot(articleSort$frequency, main = "Press Mentions of Data Science Skills", ylab = "# of mentions", names.arg = articleSort$skill, col = rainbow(nrow(articleSort)), horiz = FALSE)
...
op <- par(mar = c(4.2,6.3,3,2) + 0.1)
barplot(twitSort1$t_freq[15:1], main = "The 15 Most Mentioned Data Science Skills on Twitter I", xlab = "# of Twitter Mentions", names.arg = twitSort1$skill_title[15:1], las = 2, col = rainbow(15), cex.names = 0.8, horiz = TRUE)
par(op)

op <- par(mar = c(4.2,6.3,3,2) + 0.1)
barplot(twitSort2$t_freq[15:1], main = "The 15 Most Mentioned Data Science Skills on Twitter II", xlab = "# of Twitter Mentions", names.arg = twitSort2$skill_title[15:1], las = 2, col = rainbow(15), cex.names = 0.8, horiz = TRUE)
par(op)

op <- par(mar = c(4.2,6.3,3,2) + 0.1)
barplot(twitSort3$t_freq[15:1], main = "The 15 Most Mentioned Data Science Skills on Twitter III", xlab = "# of Twitter Mentions", names.arg = twitSort3$skill_title[15:1], las = 2, col = rainbow(15), cex.names = 0.8, horiz = TRUE)
par(op)

op <- par(mar = c(4.2,6.3,3,2) + 0.1)
barplot(twitSort4$t_freq[15:1], main = "The 15 Most Mentioned Data Science Skills on Twitter IV", xlab = "# of Twitter Mentions", names.arg = twitSort4$skill_title[15:1], las = 2, col = rainbow(15), cex.names = 0.8, horiz = TRUE)
par(op)
...
x <- barplot(top15$frequency, main = "Distribution of Skills", xlab = "skills", ylab="frequency", col=c("darkblue","red"), names.arg=top15$skill_name)


barplot(top15$frequency, main="Dis. of Skills", ylab="frequency", names.arg=top15$skill_name, las=2, col=rainbow(15))

p <- qplot(top15$skill_name, top15$frequency, data = top15, color = rainbow(15))
p

p + theme("Distribution of skills" = element_text(angle = 90, hjust = 1))
'
...
'
set.seed(1234)
wordcloud(words = articleSort$skill_name, freq = articleSort$frequency, rot.per=0.45, colors=brewer.pal(8, "Dark2"))
'
```
...
In any firm seeking to turn data into insight, data science is essential. But, it's hard to find experts with all the necessary talents - it may be necessary to grow them in-house; if you do find one, they must be kept engaged, and have the autonomy to design their own solutions, to turn data into prosperity, and transform the fate of a firm.
