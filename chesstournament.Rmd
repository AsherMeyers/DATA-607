...
colnames(TournamentInfo) <- c("Pair", "Name", "Total", "R1", "R2", "R3", "R4", "R5", "R6", "R7")
)
View(TournamentInfo)
```
...
for (i in 2:64) {
  Opponents <- rbind(Opponents, c(i, as.numeric(str_extract(NameRows[i,4:10],"[0-9]{1,2}"))))
}
View(Opponents)
```
...
for (i in 1:64) {
  OppRatings$GamesPlayed[i] <- 7 - sum(is.na(OppRatings[i,2:7]))
