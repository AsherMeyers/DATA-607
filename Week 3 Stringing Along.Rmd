...
Problem 3. Let's encode our original string of names
...
First, identify the names that have their last name first.
```{r}
lastfirst <- str_detect(name,",")
lastfirst
```

Split names separated by a comma into two vectors
```{r}
...
for (i in 1:length(name)) {
  if (lastfirst[i]) { 
  splitname[i]<- str_trim(paste(rev(unlist(splitname[i])), collapse = " "))
  }
}
